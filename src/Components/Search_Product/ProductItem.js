...
export default function ProductItem({ product }) {
  ...
  const handlewishlist = () => {
    if (userId === null) {
      ...
      if (addedToWishlist) {
        axios
          .delete(`http://localhost:8080/flipkart/wishlist/remove?userId=${userId}&productId=${product.product_id}`)
          .then(() => {
        ...
        axios
          .post(`http://localhost:8080/flipkart/wishlist/add?userId=${userId}&productId=${product.product_id}`)
          .then(() => {
  ...
  const handlecart = () => {
    if (userId === null) {
      ...
      if (addedToCart) {
        axios
          .delete(`http://localhost:8080/flipkart/cart/remove?userId=${userId}&productId=${product.product_id}`)
          .then(() => {
        ...
        axios
          .post(`http://localhost:8080/flipkart/cart/add?userId=${userId}&productId=${product.product_id}&quantity=1`)
          .then(() => {
